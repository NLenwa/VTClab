...
float partner_val_fuel = 1;
float partner_val_money = 1;
int res_taken;
...
void SetPartnerVal(int resource)
{
	if (resource == FUEL)
	{
		partner_val_fuel = 0.5;
	}
	else if (resource == MONEY)
	{
		partner_val_money = 0.5;
	}
}
	...
	for (long i = 0; i < liczba_prz_w_prom; i++)
	{
			...
			((prz->vPos - state.vPos + Vector3(0, state.vPos.y - prz->vPos.y, 0)).length() < radius))
		{
			...
			if (prz->type == ITEM_COIN)
			{
				...
				{
					res_taken = MONEY;
					taking_value = (float)value * money_collection_skills * partner_val_money;
			...
			{
				res_taken = FUEL;
				taking_value = (float)value * fuel_collection_skills * partner_val_fuel;
